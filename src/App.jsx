...
export default function App() {
  ...
  const [bridalPartySelections, setBridalPartySelections] = React.useState({});
  ...
  const onNext = async () => {
    ...
    // 🔥 CRITICAL: Sync React state to form values BEFORE any checks
    console.log("🔍 Syncing state to form - selectedDates:", selectedDates);
    console.log("🔍 Syncing state to form - bridalPartySelections:", bridalPartySelections);
    
    // ✅ FIX: Check if selectedDates is an array with length
    if (Array.isArray(selectedDates) && selectedDates.length > 0) {
      setValue("selectedDates", selectedDates);
      console.log("✅ Set selectedDates in form:", selectedDates);
    }
    
    // ✅ FIX: Check if bridalPartySelections is an object with keys
    if (bridalPartySelections && typeof bridalPartySelections === 'object' && Object.keys(bridalPartySelections).length > 0) {
      setValue("bridalPartySelections", bridalPartySelections);
      console.log("✅ Set bridalPartySelections in form:", bridalPartySelections);
    }

    // 🔥 NOW get serviceType AFTER sync
    const serviceType = getValues("service_type");
    ...
    console.log("🔍 Contact step check:", {
      isContactStep,
      region,
      serviceType,
      step,
      condition1: serviceType === "Non-Bridal" && step === 7,
      condition2: serviceType === "Bridal" && step === 8,
      condition3: serviceType === "Semi-Bridal" && step === 8,
      condition4: region === "Destination Wedding" && step === 5
    });
    ...
    if (isContactStep) {
      ...
      try {
        ...
        console.log("🔍 Form data after sync:", data);
        console.log("🔍 selectedDates from form:", data.selectedDates);
        console.log("🔍 bridalPartySelections from form:", data.bridalPartySelections);
        ...
        const bookingData = {
          ...
          non_bridal_saree_draping_count: data.nonBridal?.sareeDrapingCount || 0,
          non_bridal_has_hijab_setting: data.nonBridal?.hasHijabSetting,
          non_bridal_hijab_setting_count: data.nonBridal?.hijabSettingCount || 0,
          ...
          // ✅ MULTIPLE DATES DATA - now from form, not state
          selectedDates: data.selectedDates || [],
          bridalPartySelections: data.bridalPartySelections || {},
        };

        console.log("🔍 Final booking data being sent:", bookingData);

        let response;
        const isMultiDay = Array.isArray(bookingData.selectedDates) && bookingData.selectedDates.length > 1;
        const endpoint = isMultiDay ? '/multi-day-bookings' : '/bookings';
        
        console.log(`✅ Using ${isMultiDay ? 'multi-day' : 'single-day'} booking endpoint`);
        
        if (isEditMode && editingBookingId) {
          console.log("Updating existing booking:", editingBookingId);
          response = await api.put(`${endpoint}/${editingBookingId}`, bookingData);
        } else {
          console.log("Creating new booking with data:", bookingData);
          response = await api.post(endpoint, bookingData);
        }

        // Handle destination weddings differently
        if (region === "Destination Wedding") {
          console.log("Destination wedding booking saved, continuing to Calendly step");
          setStep((s) => Math.min(s + 1, totalSteps));
          return;
        }

        // Show loader and redirect to quote page
        const bookingId = response.data.booking_id;
        if (bookingId) {
          const loader = document.createElement("div");
          loader.className = "fixed inset-0 flex items-center justify-center bg-black/90 z-50";
          loader.innerHTML =
          ...
          window.showToast("Thank you! Please check your email for package options and next steps.", "success");
        }
        ...
        window.showToast("Failed to save your information. Please try again.", "error");
        return;
      }
    }
    ...
    if (isQuoteStep) {
      const data = getValues();
      console.log("🔍 Frontend: Quote data being sent:", JSON.stringify(data, null, 2));
                    ...
                    <>
                      ...
                      {step === 6 && (
                        <BrideAddons
                          ...
                          setValue={setValue}
                          errors={errors}
                          selectedDates={selectedDates} // ✅ add this
                        />
                      )}
                      ...
                      {step === 7 && (
                        <BridalParty
                          ...
                          setValue={setValue}
                          errors={errors}
                          selectedDates={selectedDates} // ✅ show date chips
                          bridalPartySelections={bridalPartySelections} // ✅ pass current selections
                          setBridalPartySelections={setBridalPartySelections} // ✅ update handler
                        />
                      )}
